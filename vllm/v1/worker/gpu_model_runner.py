...
from vllm.model_executor.model_loader import get_model_loader, TensorizerLoader
...
from vllm.v1.sample.logits_processor import build_logitsprocs, LogitsProcessors
...
class GPUModelRunner(LoRAModelRunnerMixin, KVConnectorModelRunnerMixin):
    ...
    def model_forward(
        ...
        **model_kwargs,
        ...
        This method can be overridden by subclasses to customize model execution.
    ...
    def load_model(self, eep_scale_up: bool = False) -> None:
        ...
        if (
            ...
            self.model.compile(
                fullgraph=envs.VLLM_TEST_DYNAMO_FULLGRAPH_CAPTURE, backend=backend
            )
            return
    ...
    def _dummy_run(
        ...
        if ubatch_enabled:
            should_ubatch = (
                num_tokens >= self.parallel_config.dbo_decode_token_threshold
        ...
        if should_ubatch:
            ...
            ubatch_slices = [
                UBatchSlice(slice(0, num_reqs // 2), slice(0, num_tokens // 2)),
                UBatchSlice(
                    slice(num_reqs // 2, num_reqs), slice(num_tokens // 2, num_tokens)
                ),
            ]
        ...
        def create_attn_groups(
            ...
            for attn_backend, layer_names in attn_backends_map.items():
                ...
                attn_group = AttentionGroup(
                    attn_backend, attn_metadata_builders, layer_names
                )
    ...
    def _kv_cache_spec_attn_group_iterator(
        self,
    ) -> Iterator[tuple[KVCacheSpec, AttentionGroup]]:
